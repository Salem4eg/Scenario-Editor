...
ProvinceBasePopulationDialog::ProvinceBasePopulationDialog(QWidget *parent)
	: QDialog(parent), m_hasChanges(false), m_currentMaxPopId(-1)
{
	...
	auto* popGroupsScrollArea = new QScrollArea;
	...
	mainLayout->addWidget(popGroupsScrollArea);
	...
	connect(closeButton, &QPushButton::pressed, this, &ProvinceBasePopulationDialog::closeWidget);
...
ProvinceBasePopulationDialog::~ProvinceBasePopulationDialog()
{}

void ProvinceBasePopulationDialog::setPopData(const QList<PopData>& pops)
{
	m_hasChanges = false;
	m_pops.clear();
...
void ProvinceBasePopulationDialog::updateCharts()
{
	clearPopGroups();
...
void ProvinceBasePopulationDialog::setCurrentMaxPopId(int popId)
{
	m_currentMaxPopId = popId;
}

void ProvinceBasePopulationDialog::setCultures(const QList<DemographicCategory>& cultures)
{
...
void ProvinceBasePopulationDialog::setTypes(const QList<DemographicCategory>& types)
{
...
void ProvinceBasePopulationDialog::setReligions(const QList<DemographicCategory>& religions)
{
...
void ProvinceBasePopulationDialog::setIdeologies(const QList<DemographicCategory>& ideologies)
{
	...
	for (const auto& demographic_category: ideologies)
	{
		...
		ideology.id = normal_ideologies.size();
...
QList<PopData> ProvinceBasePopulationDialog::getPopData() const
{
	return m_pops.values();
}

int ProvinceBasePopulationDialog::getCurrentMaxPopId() const
{
	return m_currentMaxPopId;
}

void ProvinceBasePopulationDialog::addPopGroup(const PopData& pop)
{
...
void ProvinceBasePopulationDialog::setCulturesChart(QList<ChartData> cultures)
{
	m_culturesChart->setPopData(cultures, "Cultures");
}

void ProvinceBasePopulationDialog::setTypesChart(QList<ChartData> types)
{
	m_typesChart->setPopData(types, "Types");
}

void ProvinceBasePopulationDialog::setReligionsChart(QList<ChartData> religions)
{
	m_religionsChart->setPopData(religions, "Religions");
}

int ProvinceBasePopulationDialog::getNewPopId()
{
	return m_currentMaxPopId++;
}

void ProvinceBasePopulationDialog::clearPopGroups()
{
